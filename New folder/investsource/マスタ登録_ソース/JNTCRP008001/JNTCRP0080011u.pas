...
//*										ﾌｫｰｶｽ移動ができないように修正
//*		2002/11/25	HIS0003	Y.Ichihashi	検索エクスプローラでコード順が選択されている時はコード順
//*										連想順のときは連想順で出力されるように対応
//*		2002/11/29	HIS0004	Y.Ichihashi	開始・終了コード欄の連想順対応
//*     2003/12/16  HIS0005 Y.Ichihashi 得意先、仕入先の採用チェックを指定して印刷するように対応
...
uses
  ...
  VCL.StdCtrls, VCL.ExtCtrls, FireDAC.Comp.Client, Buttons,
  MJSCommonu, MJSDBModuleu, MJSDispCtrl, MJSQuery,
  MJSLabel, MJSEdits, MJSPanel, MJSComboBox, MJSGroupBox, MJSCheckBox,
  MJSBitBtn, MJSRadioButton,
  MJSKeyDataState,  //<Shift> ADD
  JNTMasWndIFu;

type
{$include ActionInterface.inc} 			 		// ｱｸｼｮﾝｲﾝﾀｰﾌｪｲｽの追加
  TJNTCRP0080011f = class(TForm)
    MPanel1: TMPanel;
    MLabel1: TMLabel;
    ...
    CNoCheck: TMCheckBox;
    procedure fnJNTCRP0080011fOnCreate(Sender: TObject);
...
var
	JNTCRP0080011f: TJNTCRP0080011f;
...
constructor	TJNTCRP0080011f.fnCreateDialog (pRec: Pointer; AOwner: TComponent; cDataModule: TMDataModulef; cDBSelect: TFDConnection;
										nMasterDivision: Integer; nCodeDigit: Integer; nCodeAttribute: Integer; strItemName: String);
begin
	m_pRec				:= pRec;
...
procedure	TJNTCRP0080011f.CMChildkey (var Msg: TWMKey);
var
...
procedure	TJNTCRP0080011f.fnJNTCRP0080011fOnCreate (Sender: TObject);
var
	strFormat: String;

begin
	MJSColorChange (TJNTCRP0080011f (Self), 0, 0, 0, 0, rcCOMMONAREA (m_pRec^.m_pCommonArea^).SysFocusColor);
...
procedure	TJNTCRP0080011f.fnDialogCommonFunctionOnKeyDown (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//***********************************************************************************************************/
//*																											*/
//*	関数	: TJNTCRP0080011f.fnDialogComboboxFunctionOnKeyDown (TObject; var Word; TShiftState)				*/
//*																											*/
...
//*																											*/
//***********************************************************************************************************/
procedure	TJNTCRP0080011f.fnDialogComboboxFunctionOnKeyDown (Sender: TObject; var Key: Word; Shift: TShiftState);
var
...
procedure	TJNTCRP0080011f.fnDialogComboBoxListKindOnChange (Sender: TObject);
begin
...
procedure	TJNTCRP0080011f.DialogComboBoxListKindOnKeyDown (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//*																											*/
//***********************************************************************************************************/
procedure	TJNTCRP0080011f.fnDialogEditExCodeStartOnExit (Sender: TObject);
var
	sGCode	:	String;
begin
	if	(m_nCodeAttribute <= 1) then
...
//*																											*/
//***********************************************************************************************************/
procedure	TJNTCRP0080011f.fnDialogEditExCodeEndOnExit (Sender: TObject);
var
	sGCode	:	String;
begin
	if	(m_nCodeAttribute <= 1) then
...
//*																									*/
//***************************************************************************************************/
function	TJNTCRP0080011f.fnChangeSelectionScreen (): Boolean;
var
...
//*                                                                                                 */
//***************************************************************************************************/
function	TJNTCRP0080011f.fnGetRequestExCode (nAccessMethod: Integer): String;
var
...
//*                                                                                                 */
//***************************************************************************************************/
procedure	TJNTCRP0080011f.fnSetDepartmentName (nSetPosition: Integer; Sender: TObject);
var
	dqMasterData	 : TMQuery;
...
//*                                                                                                 */
//***************************************************************************************************/
function	TJNTCRP0080011f.fnGetDepartmentName (strExCode: String): String;
var
	dqMasterData	 : TMQuery;
	strDepartmentName: String;
...
procedure	TJNTCRP0080011f.fnDialogButtonOkOnClick (Sender: TObject);
begin
	if	((m_nPrintDivision >= 1) and (m_nMasterDivision <> 22)) then
    ...
    begin
        ...
        begin
            MjsMessageBoxEX(Self,'採用状態をどれかチェックしてください','採用状態',mjInformation,mjOk,mjDefOk);
            Exit;
        end;
...
//***************************************************************************************************/
//*                                                                                                 */
//*	関数	: TJNTCRP0080011f.fnDialogEditExCodeStartOnArrowClick (TObject)								*/
//*																									*/
...
//*																									*/
//***************************************************************************************************/
procedure	TJNTCRP0080011f.fnDialogEditExCodeStartOnArrowClick (Sender: TObject);
var
...
//***************************************************************************************************/
//*                                                                                                 */
//*	関数	: TJNTCRP0080011f.fnDialogEditExCodeEndOnArrowClick (TObject)								*/
//*																									*/
...
//*																									*/
//***************************************************************************************************/
procedure	TJNTCRP0080011f.fnDialogEditExCodeEndOnArrowClick (Sender: TObject);
var
	cMasWndParam: TJNTMasWndParam;
	cMasWndIF	: TJNTMasWndIF;
...
//***************************************************************************************************/
//*                                                                                                 */
//*	関数	: TJNTCRP0080011f.fnGetSQLConstruction (Integer)											*/
//*																									*/
...
//*                                                                                                 */
//***************************************************************************************************/
function	TJNTCRP0080011f.fnGetSQLConstruction (nAccessMethod: Integer): String;
var
	strSQLConst	  : String;
...
function TJNTCRP0080011f.fnCodeCheck(sGCode: String): Boolean;
var
 	dqMasterData	:	TMQuery;
begin
	dqMasterData	:= TMQuery.Create (Self);
...
procedure TJNTCRP0080011f.fnTorihikisaki(Flag: Boolean);
begin
    if Flag = True then
    begin
