object JNTCRP004001NotSamef: TJNTCRP004001NotSamef
  Left = 758
  Top = 396
  Width = 593
  Height = 536
  Caption = '取引先選択'
  Color = clWhite
  Font.Charset = SHIFTJIS_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'ＭＳ Ｐゴシック'
  Font.Style = []
  Icon.Data = {
    
    00000000000000000000000000000000000000000000}
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object MLabel1: TMLabel
    Left = 16
    Top = 16
    Width = 269
    Height = 19
    AutoSize = False
    Caption = '検索条件に該当する取引先は、見つかりません。'
    FontRatio.Auto = False
  end
  object LLbl_Seishiki: TMLabel
    Left = 125
    Top = 119
    Width = 429
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object MLabel3: TMLabel
    Left = 28
    Top = 146
    Width = 26
    Height = 19
    AutoSize = False
    Caption = '敬称'
    FontRatio.Auto = False
  end
  object MLabel4: TMLabel
    Left = 28
    Top = 173
    Width = 52
    Height = 19
    AutoSize = False
    Caption = 'フリガナ'
    FontRatio.Auto = False
  end
  object MLabel5: TMLabel
    Left = 28
    Top = 200
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '簡略名称'
    FontRatio.Auto = False
  end
  object MLabel6: TMLabel
    Left = 28
    Top = 227
    Width = 26
    Height = 19
    AutoSize = False
    Caption = '連想'
    FontRatio.Auto = False
  end
  object MLabel7: TMLabel
    Left = 28
    Top = 254
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '郵便番号'
    FontRatio.Auto = False
  end
  object MLabel8: TMLabel
    Left = 28
    Top = 281
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '住所上段'
    FontRatio.Auto = False
  end
  object MLabel9: TMLabel
    Left = 28
    Top = 308
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '住所下段'
    FontRatio.Auto = False
  end
  object MLabel10: TMLabel
    Left = 28
    Top = 335
    Width = 85
    Height = 19
    AutoSize = False
    Caption = '電話番号（１）'
    FontRatio.Auto = False
  end
  object MLabel11: TMLabel
    Left = 28
    Top = 362
    Width = 85
    Height = 19
    AutoSize = False
    Caption = '電話番号（２）'
    FontRatio.Auto = False
  end
  object MLabel12: TMLabel
    Left = 28
    Top = 389
    Width = 56
    Height = 19
    AutoSize = False
    Caption = 'FAX番号'
    FontRatio.Auto = False
  end
  object MLabel13: TMLabel
    Left = 28
    Top = 416
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '代表者名'
    FontRatio.Auto = False
  end
  object MLabel14: TMLabel
    Left = 28
    Top = 119
    Width = 52
    Height = 19
    AutoSize = False
    Caption = '正式名称'
    FontRatio.Auto = False
  end
  object LLbl_Keisyou: TMLabel
    Left = 125
    Top = 146
    Width = 90
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Trcode: TMLabel
    Left = 28
    Top = 92
    Width = 78
    Height = 19
    AutoSize = False
    Caption = '取引先コード'
    FontRatio.Auto = False
  end
  object LLbl_Hurigana: TMLabel
    Left = 125
    Top = 173
    Width = 312
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Kanryaku: TMLabel
    Left = 125
    Top = 200
    Width = 164
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Rensou: TMLabel
    Left = 125
    Top = 227
    Width = 90
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Post1: TMLabel
    Left = 125
    Top = 254
    Width = 27
    Height = 19
    Alignment = taCenter
    AutoSize = False
    Color = clWhite
    ParentColor = False
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Post2: TMLabel
    Left = 177
    Top = 254
    Width = 38
    Height = 19
    Alignment = taCenter
    AutoSize = False
    Color = clWhite
    ParentColor = False
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Zyusyo1: TMLabel
    Left = 125
    Top = 281
    Width = 429
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Zyusyo2: TMLabel
    Left = 125
    Top = 308
    Width = 429
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Tel1: TMLabel
    Left = 125
    Top = 335
    Width = 164
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Tel2: TMLabel
    Left = 125
    Top = 362
    Width = 164
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Fax: TMLabel
    Left = 125
    Top = 389
    Width = 164
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object LLbl_Daihyou: TMLabel
    Left = 125
    Top = 416
    Width = 164
    Height = 19
    AutoSize = False
    Color = clWhite
    ParentColor = False
    MLeftMargin = 3
    BorderLine = tmb3DDown
    FontRatio.Auto = False
  end
  object MLabel28: TMLabel
    Left = 163
    Top = 254
    Width = 7
    Height = 19
    AutoSize = False
    Caption = '―'
    FontRatio.Auto = False
  end
  object MLabel29: TMLabel
    Left = 16
    Top = 43
    Width = 321
    Height = 19
    AutoSize = False
    Caption = '上書き取り込みをする場合は、取引先を選択してください。'
    FontRatio.Auto = False
  end
  object BBtn_Sinki: TMBitBtn
    Left = 242
    Top = 457
    Width = 105
    Height = 25
    Caption = '新規取込(&O)'
    TabOrder = 2
    OnClick = BBtn_SinkiClick
    Glyph.Data = {
      
      000000000000000000000000000000000000}
    NumGlyphs = 2
  end
  object BBtn_Uwagaki: TMBitBtn
    Left = 349
    Top = 457
    Width = 105
    Height = 25
    Caption = '上書取込(&U)'
    Enabled = False
    TabOrder = 3
    OnClick = BBtn_UwagakiClick
    Glyph.Data = {
      
      000000000000000000000000000000000000}
    NumGlyphs = 2
  end
  object BBtn_Cancel: TMBitBtn
    Left = 456
    Top = 457
    Width = 105
    Height = 25
    Caption = 'キャンセル(&C)'
    TabOrder = 4
    OnClick = BBtn_CancelClick
    Glyph.Data = {
      
      000000000000000000000000000000000000}
    NumGlyphs = 2
  end
  object BChk_Uwagaki: TMCheckBox
    Left = 355
    Top = 92
    Width = 207
    Height = 19
    Caption = '申請書の基本情報を上書きする。'
    Enabled = False
    TabOrder = 1
  end
  object ETxt_Trcode: TMTxtEdit
    Tag = 22
    Left = 125
    Top = 92
    Width = 144
    Height = 19
    ArrowDisp = adFocused
    SelectDisp = sdNone
    OnArrowClick = ETxt_TrcodeArrowClick
    ArrowType = atOmission
    AutoSize = False
    Hankaku = True
    FontRatio.Auto = False
    TabOrder = 0
    OnExit = ETxt_TrcodeExit
  end
  object MQuery1: TMQuery
    MakeSQL = True
    LockRow = False
    Left = 532
    Top = 16
  end
end
