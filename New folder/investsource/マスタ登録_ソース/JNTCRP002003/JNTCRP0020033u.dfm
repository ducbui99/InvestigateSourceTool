object JNTCRP0020033f: TJNTCRP0020033f
  Left = 404
  Top = 378
  BorderStyle = bsDialog
  Caption = 'çÃópèÓïÒê›íË'
  ClientHeight = 132
  ClientWidth = 499
  Color = clBtnFace
  Font.Charset = SHIFTJIS_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'ÇlÇr ÇoÉSÉVÉbÉN'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = fnJNTCRP0020033fOnCreate
  PixelsPerInch = 96
  TextHeight = 12
  object DialogButtonOk: TMBitBtn
    Left = 282
    Top = 104
    Width = 105
    Height = 25
    Caption = '&OK'
    TabOrder = 2
    OnClick = fnDialogButtonOkOnClick
    Glyph.Data = {
      
      898989895D86D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1D1}
  end
  object DialogButtonCancel: TMBitBtn
    Left = 390
    Top = 104
    Width = 105
    Height = 25
    Caption = 'ÉLÉÉÉìÉZÉã(&C)'
    ModalResult = 2
    TabOrder = 3
    OnClick = fnDialogButtonCancelOnClick
    Glyph.Data = {
      
      D1D1D1D1D1FFC4A6D1D1D1D1FFFFC4D1D1D1D1D1D1D1D1D1D1D1}
  end
  object MPanel1: TMPanel
    Left = 4
    Top = 105
    Width = 271
    Height = 22
    BevelOuter = bvLowered
    TabOrder = 0
    object DialogLabelMessage: TMLabel
      Left = 8
      Top = 2
      Width = 255
      Height = 18
      AutoSize = False
      Caption = 'ÉRÅ[ÉhëÆê´ãyÇ—åÖêîÇïœçXÇ∑ÇÈÇ±Ç∆Ç™Ç≈Ç´Ç‹Ç∑ÅB'
      Font.Charset = SHIFTJIS_CHARSET
      Font.Color = clBtnText
      Font.Height = -12
      Font.Name = 'ÇlÇr ÇoÉSÉVÉbÉN'
      Font.Style = []
      ParentFont = False
      FontRatio.Auto = False
    end
  end
  object MPanel2: TMPanel
    Left = 4
    Top = 4
    Width = 491
    Height = 95
    TabOrder = 1
    object DialogLabelGroupAdoption: TMLabel
      Left = 16
      Top = 224
      Width = 457
      Height = 3
      AutoSize = False
      Color = clBtnShadow
      ParentColor = False
      Visible = False
      FontRatio.Auto = False
    end
    object DialogLabelDescAdoption: TMLabel
      Left = 20
      Top = 228
      Width = 173
      Height = 19
      AutoSize = False
      Caption = 'XXXXXXXXXXXXXXçÃópãÊï™'
      Font.Charset = SHIFTJIS_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'ÇlÇr ÇoÉSÉVÉbÉN'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
      FontRatio.Auto = False
    end
    object DialogRadioButtonAdoption01: TMRadioButton
      Left = 56
      Top = 248
      Width = 101
      Height = 19
      Caption = 'P/LÇÃÇ›çÃóp'
      Checked = True
      TabOrder = 0
      TabStop = True
      Visible = False
      OnClick = fnDialogRadioButtonAdoption01OnClick
      OnKeyDown = fnDialogCommonFunctionOnKeyDown
    end
    object DialogRadioButtonAdoption02: TMRadioButton
      Left = 172
      Top = 248
      Width = 121
      Height = 19
      Caption = 'B/SÅïP/LçÃóp'
      TabOrder = 1
      Visible = False
      OnClick = fnDialogRadioButtonAdoption02OnClick
      OnKeyDown = fnDialogCommonFunctionOnKeyDown
    end
    object DialogRadioButtonAdoption03: TMRadioButton
      Left = 292
      Top = 248
      Width = 85
      Height = 19
      Caption = 'çÃópÇµÇ»Ç¢'
      TabOrder = 2
      Visible = False
      OnClick = fnDialogRadioButtonAdoption03OnClick
      OnKeyDown = fnDialogCommonFunctionOnKeyDown
    end
    object MGroupBox1: TMGroupBox
      Left = 16
      Top = 16
      Width = 459
      Height = 59
      Caption = 'ÉRÅ[ÉhåÖêîÅ^ëÆê´'
      TabOrder = 3
      object DialogLabelDescCodeDigit: TMLabel
        Left = 22
        Top = 25
        Width = 61
        Height = 19
        AutoSize = False
        Caption = 'ÉRÅ[ÉhåÖêî'
        FontRatio.Auto = False
      end
      object DialogLabelDescCodeDigitAddition: TMLabel
        Left = 118
        Top = 25
        Width = 61
        Height = 19
        AutoSize = False
        Caption = 'ÅiÇRÅ`ÇPÇOåÖÅj'
        FontRatio.Auto = False
      end
      object DialogLabelDescCodeAttribute: TMLabel
        Left = 206
        Top = 25
        Width = 61
        Height = 19
        AutoSize = False
        Caption = 'ÉRÅ[ÉhëÆê´'
        FontRatio.Auto = False
      end
      object DialogLabelDescCodeAttributeAddition: TMLabel
        Left = 306
        Top = 25
        Width = 133
        Height = 19
        AutoSize = False
        Caption = 'XXXXXXXXXXXXXXXXXX'
        FontRatio.Auto = False
      end
      object DialogEditCodeDigit: TMNumEdit
        Left = 86
        Top = 24
        Width = 25
        Height = 19
        ArrowDisp = adNone
        SelectDisp = sdNone
        ArrowType = atArrow
        AutoSize = False
        Font.Charset = SHIFTJIS_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'ÇlÇr Çoñæí©'
        Font.Style = []
        ImeMode = imClose
        FontRatio.Auto = False
        ParentFont = False
        TabOrder = 0
        OnKeyDown = fnDialogCommonFunctionOnKeyDown
        Negative = False
        Validate = False
        Value = 10
      end
      object DialogEditCodeAttribute: TMNumEdit
        Left = 270
        Top = 24
        Width = 17
        Height = 19
        ArrowDisp = adFocused
        SelectDisp = sdArrowClick
        SelectItems.Strings = (
          '0: êîéö'
          '1: êîéöÅiëOÉ[ÉçÇ†ÇËÅj'
          '2: ÉtÉäÅ[')
        ArrowType = atArrow
        F4Arrow = True
        AutoSize = False
        Font.Charset = SHIFTJIS_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'ÇlÇr Çoñæí©'
        Font.Style = []
        ImeMode = imDisable
        FontRatio.Auto = False
        ParentFont = False
        TabOrder = 1
        OnChange = fnDialogEditCodeAttributeOnChange
        OnKeyDown = fnDialogCommonFunctionOnKeyDown
        Negative = False
        MaxValue = 2
        Digits = 1
        Validate = False
        Zero = True
      end
    end
    object Panel1: TMPanel
      Left = 8
      Top = 8
      Width = 473
      Height = 81
      Caption = 'ñ¢égóp'
      Font.Charset = SHIFTJIS_CHARSET
      Font.Color = clWindowText
      Font.Height = -107
      Font.Name = 'ÇlÇr ÇoÉSÉVÉbÉN'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
    end
  end
end
